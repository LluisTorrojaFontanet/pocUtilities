package ad.au.com.southsky.jfreesane;
 ...
 * {@link ScanListenerAdapter} instead of implementing this interface.
 *
 * <p>
 * If you are using {@code ScanListener} to update a user interface, the frequency of notifications
 * (specifically from {@link #recordRead}) may be too frequent. In this case, you can wrap your
 * listener using one of the methods in {@link RateLimitingScanListeners} to obtain a listener that
 ...
 * <li>{@code frameAcquisitionStarted} -- once for each frame. Typically there is only one frame per
 * page, but in older three-pass color scanners (where one scan pass is made per color), there will
 * be three frames per page</li>
 * <li>{@code recordRead} -- once for each record, one or more records make up a frame</li>
 * <li>{@code scanningFinished} -- once a page has been scanned</li>
 * </ol>
 *
 * <p>
 * JFreeSane attempts to report accurate numbers to listeners, but this is not possible in all
